...
(如果在子组件中分别发送ajax请求,则需要发送多次请求,对于性能造成影响,因此在父组件中发送请求)
...
export default{
		...
		mounted () {
			this.getHomeInfo();   生命周期函数执行 this.getHomeInfo()
		},
		methods:{            定义方法
			getHomeInfo(){
				axios.get('/api/index.json')    axios.get()请求后.then()执行输出
...
4/ 在本地的static目录下创建本地访问的数据文件 index.json
...
5/ 将老师准备好的index.json文件(模拟数据),放入mock文件夹中
(内容):
{
  "ret": true,
  "data": {
